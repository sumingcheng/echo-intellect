...
class RRFMerger:
    ...
    def merge_results(
        ...
        try:
            ...
            merged_lexical = self._merge_multiple_vectors(lexical_results)
            
            # 2. 记录各结果的排名
            data_scores = defaultdict(lambda: {"embedding_rank": None, "lexical_rank": None, "metadata": {}})
            ...
            # 处理合并后的词法检索结果
            for rank, result in enumerate(merged_lexical, 1):
                data_id = result.data_id if hasattr(result, 'data_id') else result.metadata.get('data_id', '')
                if data_id:  # 只处理有效的data_id
                    data_scores[data_id]["lexical_rank"] = rank
                if "content" not in data_scores[data_id]:
                    data_scores[data_id]["content"] = result.content
                    data_scores[data_id]["data_id"] = data_id
                    data_scores[data_id]["tokens"] = result.tokens
                data_scores[data_id]["metadata"].update(result.metadata)
                data_scores[data_id]["lexical_score"] = result.score
            ...
            for data_id, data in data_scores.items():
                ...
                # 计算词法检索RRF得分
                if data["lexical_rank"] is not None:
                    lexical_rrf = lexical_weight / (self.k + data["lexical_rank"])
            ...
            logger.info(
                f"RRF合并完成: embedding({len(merged_embedding)}) + "
                f"lexical({len(merged_lexical)}) -> merged({len(merged_results)})"
            )
            return merged_results
            ...
            # 失败时返回简单拼接的结果
            return self._simple_merge(embedding_results, lexical_results)
    ...
    def _simple_merge(
        ...
        try:
            ...
            return embedding_results + lexical_results
